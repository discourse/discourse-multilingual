...
describe Multilingual::Language do
  ...
  describe "CustomLanguage" do
    before { Multilingual::CustomLanguage.create("abc", name: "Custom Language", run_hooks: true) }
    ...
    describe "#destroy" do
      ...
      it "should remove a language's exclusions" do
    ...
    describe "#update" do
      ...
      it "should add to both interface and content exclusions" do
        ...
        expect(Multilingual::LanguageExclusion.get(Multilingual::ContentLanguage::KEY, "fr")).to eq(
          true,
        )
      end
  ...
  describe "#get" do
    ...
    it "should return an array of languages when given an array of codes" do
      result = Multilingual::Language.get(%w[aa ab])
      expect(result.map(&:locale)).to eq(%w[aa ab])
      expect(result.map(&:name)).to eq(%w[Afar Abkhaz])
    end
  end
  ...
  describe "#filter" do
    ...
    it "should order languages by interface enabled" do
      ...
      result =
        Multilingual::Language.filter(query: "ht", order: "interface_enabled", ascending: true)
  ...
  describe "LanguageExclusion.set" do
    ...
    it "should add interface exclusions" do
      Multilingual::LanguageExclusion.set(
        "sl",
        Multilingual::InterfaceLanguage::KEY,
        enabled: false,
      )
      Multilingual::Cache.refresh!
      expect(Multilingual::LanguageExclusion.get(Multilingual::InterfaceLanguage::KEY, "sl")).to eq(
        true,
      )
    end
    ...
    it "should remove exclusions" do
      ...
      Multilingual::Cache.refresh!
      expect(Multilingual::LanguageExclusion.get(Multilingual::InterfaceLanguage::KEY, "sv")).to eq(
        false,
      )
    end
  end
  ...
  describe "#bulk_update" do
    it "should update languages" do
      Multilingual::Language.bulk_update(
        [{ locale: "it", content_enabled: false }, { locale: "fr", interface_enabled: false }],
      )
      expect(Multilingual::LanguageExclusion.get(Multilingual::ContentLanguage::KEY, "it")).to eq(
        true,
      )
      expect(Multilingual::LanguageExclusion.get(Multilingual::InterfaceLanguage::KEY, "fr")).to eq(
        true,
      )
    end
  end
  ...
  describe "#bulk_destroy" do
    before do
      ...
      Multilingual::CustomLanguage.bulk_destroy(%w[cus1 cus2])
    end
