...
describe Multilingual::AdminTranslationsController do
  ...
  let(:custom_languages) do
    "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/custom_languages.yml"
  end
  let(:category_name_translation) do
    "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/category_name.wbp.yml"
  end
  let(:category_description_translation) do
    "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/category_description.wbp.yml"
  end
  let(:server_locale) do
    "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/server.wbp.yml"
  end
  let(:client_locale) { "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/client.fr.yml" }
  let(:custom_client_locale) do
    "#{Rails.root}/plugins/discourse-multilingual/spec/fixtures/client.wbp.yml"
  end
  ...
  it "uploads category name translation" do
    ...
    post "/admin/multilingual/translations.json",
         params: {
  ...
  it "uploads client locale" do
    ...
    delete "/admin/multilingual/translations.json", params: { file_type: "client", locale: "fr" }
    expect(response.status).to eq(200)
